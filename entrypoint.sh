...
lftp $WDEFAULT_METHOD://$FTP_SERVER:$WDEFAULT_PORT -u "$FTP_USERNAME","$FTP_PASSWORD" -e "set ssl:verify-certificate false; mirror --reverse --continue --dereference $WDEFAULT_ARGS --reverse -x ^\.git/$ $WDEFAULT_LOCAL_DIR $WDEFAULT_REMOTE_DIR; quit"
